        ...
        <form method="POST" action="{{ route('admin.classes.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" id="class_date" name="class_date" value="{{ old('class_date') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    @error('class_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <input type="number" id="duration_hours" name="duration_hours" value="{{ old('duration_hours', 0) }}" min="0" placeholder="Hours" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        <div>
                            <input type="number" id="duration_minutes" name="duration_minutes" value="{{ old('duration_minutes', 0) }}" min="0" max="59" placeholder="Minutes" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                    </div>
                    @error('duration')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="number" id="payment" name="payment" value="{{ old('payment') }}" step="0.01" min="0" placeholder="Enter payment amount" class="w-full md:w-1/2 rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('payment')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
